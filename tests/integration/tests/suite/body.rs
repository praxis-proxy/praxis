...
fn released_stream_buffer_body_still_honors_global_limit() {
    ...
    RELEASE_OBSERVED.store(false, Ordering::SeqCst);
    ...
    while !RELEASE_OBSERVED.load(Ordering::SeqCst) {
        assert!(
            std::time::Instant::now() < deadline,
            "the filter must release on the first chunk before the second is sent"
        );
...
/// Set by [`ReleaseFirstChunkFilter`] when it releases, so the test can hold
/// the second chunk until the released path is the one in effect.
static RELEASE_OBSERVED: AtomicBool = AtomicBool::new(false);
