...
impl FilterPipeline {
    ...
    pub async fn execute_http_response(&self, ctx: &mut HttpFilterContext<'_>) -> Result<FilterAction, FilterError> {
        let resp_snapshot = resp_condition_snapshot(&self.filters, ctx);
...
/// Returns true if any response condition in the filter list references headers.
fn any_condition_uses_headers(filters: &[ConditionalFilter]) -> bool {
    filters.iter().any(|(_, _, rc)| {
        rc.iter().any(|c| {
            let m = match c {
...
fn resp_condition_snapshot(filters: &[ConditionalFilter], ctx: &HttpFilterContext<'_>) -> Option<crate::Response> {
    let has_any = filters.iter().any(|(_, _, rc)| !rc.is_empty());
    if !has_any {
        return None;
    }
    ctx.response_header.as_ref().map(|r| {
        let headers = if any_condition_uses_headers(filters) {
            r.headers.clone()
        } else {
            http::HeaderMap::new()
        };
