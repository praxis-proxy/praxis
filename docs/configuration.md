...
`Host` header value. Optional `headers` field restricts
matching to requests with all specified header values
present (AND semantics, case-sensitive).
...
|-------|------|---------|-------------|
| `type` | string | required | `"http"`, `"tcp"`, or `"grpc"` |
...
Enables gzip, brotli, and zstd response compression.
Each algorithm can be individually enabled/disabled with
...
At least one algorithm must be enabled. All three are
enabled by default with no config required:
...
For backward compatibility, configs using the `pipeline:`
and `routes:` shorthand are auto-converted at startup via
`apply_defaults()`. The shorthand format uses a single
shared pipeline across all listeners:

```yaml
# Shorthand format (still works)
listeners:
  - address: "0.0.0.0:8080"
