...
│   ├── route                   Legacy routing rules (backward compat)
...
├── entry                       Bridges config PipelineEntry to runtime
...
└── builtins/                   Built-in filter implementations
    ...
    │   │   └── inference/      Model routing, token counting (planned)
    │   ├── observability/
...
sequenceDiagram
    ...
    M->>M: index filter_chains: HashMap<name, &[PipelineEntry]>
    loop each Listener
...
`Vec<Cluster>`. Legacy `routes`/`pipeline` fields
