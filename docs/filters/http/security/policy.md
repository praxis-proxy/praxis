...
| `allow_private_idp` | bool | no | Allow policy-engine calls to private or loopback identity providers. Disabled by default to limit SSRF through policy-defined endpoints. This does not affect Praxis upstreams, which use `insecure_options.allow_private_endpoints`. |
