...
| `llm.max_request_bytes` | integer | no | Ceiling on a buffered inference request body, in bytes. The inference path buffers in `read_only` too — the model is in the body — so it carries its own bound rather than sharing the `max_buffer_bytes` figure sized for JSON-RPC tool calls. A body over the ceiling is rejected before any filter runs, which is the fail-closed answer for one the filter cannot read a model from. Peak memory is roughly this times the in-flight request count, so raise it deliberately. Multimodal requests carrying base64 images, long chat histories, and large embeddings inputs all run past the 1 MiB default. |
