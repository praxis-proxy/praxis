...
//!
//! - `openssl` (default) — [`rustls_openssl`], backed by the system OpenSSL
//!   library. The cryptography is performed by `libcrypto.so`, which on a
//!   FIPS-enabled host is the platform's validated module. This is the
//!   configuration Praxis targets for FIPS 140-3.
//! - `aws-lc-rs` — rustls' built-in AWS-LC provider. Statically linked, so it
//!   builds anywhere without system OpenSSL, but the module is AWS's rather
//!   than the platform's.
//!
//! ```console
//! cargo build                                                  # OpenSSL
...
compile_error!(
    "no crypto provider selected: enable exactly one of the `openssl` or `aws-lc-rs` features"
);
...
mod tests {
    ...
    fn installed_provider_resolves() {
        ...
        assert!(
            ...
            "provider must offer cipher suites"
        );
    }
