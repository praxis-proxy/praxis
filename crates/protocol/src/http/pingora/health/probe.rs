...
///
/// Long enough for a same-datacenter peer to drain and FIN (so our close is
/// a FIN, not a RST on unread bytes), short enough that a peer that never
/// closes cannot delay the next probe tick.
