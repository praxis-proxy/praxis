...
fn live_listener_meta(
    ...
    for listener in &new_config.listeners {
        ...
        if restart_blocked.contains(name) {
            // A blocked listener keeps serving its live pipeline, so its
            // chain_names must never come from the rejected new config. Prefer
            // the live metadata. If the listener was removed in an earlier
            // reload and then reintroduced with a mismatched protocol, its live
            // entry is gone, so fall back to the startup chains the original
            // bound pipeline is still serving.
            if let Some(live_entry) = live_meta.get(name) {
                entry.chain_names.clone_from(&live_entry.chain_names);
            } else {
                entry.chain_names.clone_from(&bound_listener.chain_names);
            }
        }
    }
    meta
...
"#,
    ...
    #[test]
    fn reintroduced_blocked_listener_reports_original_chains_not_rejected_config() {
        let (live, startup_config, registry, shutdown, meta, cluster_meta, bound) = setup_live_pipelines();
...
"#,
        ...
        assert!(
            ...
            "removing web from the config must drop its live metadata entry while its bound pipeline keeps serving"
        );

        let web_reintroduced_on_tcp = Config::from_yaml(
            r#"
insecure_options:
  allow_private_upstreams: true
listeners:
...
"#,
        ...
        let web = loaded
            .get("web")
            .expect("web is back in the config, so it has a metadata entry again");
        assert_eq!(
            web.chain_names,
            ["main"],
            "a listener removed then reintroduced on a mismatched protocol still serves its original bound pipeline, so its metadata must report the startup chains, not the rejected config's tcp_main"
        );
        assert_eq!(
            web.protocol,
            praxis_core::config::ProtocolKind::Http,
            "and it must report the bound HTTP protocol, not the rejected tcp"
        );
    }
...
"#,
        ...
        let bound = BoundListeners::from_config(&old_config);
