...
impl HttpFilter for TraceContextFilter {
    ...
    async fn on_request(&self, ctx: &mut HttpFilterContext<'_>) -> Result<FilterAction, FilterError> {
        ...
        let traceparent = build_traceparent(&incoming, &new_span_id, ctx);
...
fn generate_span_id(ctx: &HttpFilterContext<'_>) -> String {
    let id = ctx.id_generator.generate(ctx.time_source);
    // Take the first 16 hex chars (8 bytes) for a span ID.
    // IdGenerator produces 32 hex chars; truncate to span-ID length.
    let id: String = id.chars().take(SPAN_ID_HEX_LEN).collect();
