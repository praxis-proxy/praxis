...
use crate::{
    ...
    builtins::http::ai::openai::translation::chat_completions::{
...
fn transform_response_body(ctx: &HttpFilterContext<'_>, bytes: &Bytes) -> Option<Bytes> {
    ...
    let context = state.context.clone().with_completed_at(ctx.time_source.now().as_secs());
    let transformed = response_resource_bytes(&response, &context)?;
...
mod tests {
    ...
    use super::*;
    use crate::{HttpFilter, Rejection, body::MAX_JSON_BODY_BYTES, test_utils};
    ...
    #[tokio::test]
    async fn on_request_body_rejects_unsupported_non_function_tools() {
        let filter = make_filter();
        let request = test_utils::make_request(http::Method::POST, "/v1/responses");
        let mut ctx = test_utils::make_filter_context(&request);
        ctx.current_filter_id = Some(0);
        let original =
            Bytes::from_static(br#"{"model":"gpt-4o-mini","input":"hello","tools":[{"type":"file_search"}]}"#);
        let mut body = Some(original.clone());

        let action = filter.on_request_body(&mut ctx, &mut body, true).await.unwrap();
        let rejection = reject(action);
        let rejection_body: Value = serde_json::from_slice(rejection.body.as_deref().unwrap()).unwrap();

        assert_eq!(rejection.status, 400, "unsupported hosted tools should reject");
        let message = rejection_body["error"]["message"].as_str().unwrap();
        assert!(
            message.contains("file_search"),
            "rejection should identify the unsupported tool type"
        );
        assert_rejected_request_is_clean(&ctx, &body, original);
    }

    fn reject(action: FilterAction) -> Rejection {
        assert!(
            matches!(&action, FilterAction::Reject(_)),
            "expected unsupported hosted tools to reject"
        );
    ...
    fn assert_rejected_request_is_clean(ctx: &HttpFilterContext<'_>, body: &Option<Bytes>, original: Bytes) {
        assert_eq!(
            body,
            &Some(original),
            "rejected request body should remain untransformed"
        );
        assert!(
            ctx.extra_request_headers.is_empty(),
            "rejected request should not inject content-length"
        );
        assert!(
            ctx.get_filter_state::<ResponsesToChatState>().is_none(),
            "rejected request should not capture response transform state"
        );
    }
    ...
    #[tokio::test]
    async fn on_response_body_uses_response_time_for_completed_at() {
        let filter = make_filter();
        let request = test_utils::make_request(http::Method::POST, "/v1/responses");
        let request_time = FixedTimeSource::new(Duration::from_secs(100));
        let response_time = FixedTimeSource::new(Duration::from_secs(123));
        let mut ctx = test_utils::make_filter_context(&request);
        ctx.current_filter_id = Some(0);
        ctx.time_source = &request_time;
        let mut request_body = Some(Bytes::from_static(br#"{"model":"gpt-4o-mini","input":"hello"}"#));
        let action = filter.on_request_body(&mut ctx, &mut request_body, true).await.unwrap();
        assert!(
            matches!(action, FilterAction::Continue),
        ...
        ctx.time_source = &response_time;
        let mut response = test_utils::make_response();
        ctx.response_header = Some(&mut response);
        let mut response_body = Some(chat_completion_body());

        let action = filter.on_response_body(&mut ctx, &mut response_body, true).unwrap();

        assert!(
            matches!(action, FilterAction::Continue),
            "response body phase should continue"
        );
        let transformed: Value = serde_json::from_slice(response_body.as_ref().unwrap()).unwrap();
        assert_eq!(transformed["created_at"], 100, "created_at should use request time");
        assert_eq!(
            transformed["completed_at"], 123,
            "completed_at should use response completion time"
        );
    }

    fn chat_completion_body() -> Bytes {
        Bytes::from_static(
            br#"{
                "id": "chatcmpl_123",
