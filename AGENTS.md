...
- Avoid cloning external Rust repositories unless
  the needed code is not available from this checkout,
  Cargo metadata, docs.rs, crates.io package sources,
  the local Cargo registry, or local path/git
  dependencies already referenced by `Cargo.toml`
  or `Cargo.lock`. If cloning is still needed, keep
  it outside this repository unless asked otherwise
  and explain why local sources were insufficient.
